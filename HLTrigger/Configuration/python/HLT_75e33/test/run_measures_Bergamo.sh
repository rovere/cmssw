...
run_hlt() {
  ...
  for config_name in "${hlt_config_names[@]}"; do
    ...
    streams=8
    events=1000
    logdir="logs.Bergamo.HLT.$config_name.${jobs}j.${threads}t.${streams}s"
...
run_ofp2() {
  for config_name in "${ofp2_config_names[@]}"; do
    ...
    streams=64
    events=1000
    logdir="logs.Bergamo.OFP2.$config_name.${jobs}j.${threads}t.${streams}s"
    ...
    patatrack-scripts/benchmark -j ${jobs} -t ${threads} -s ${streams} -e ${events} --run-io-benchmark \
      ...
      --slot n=1:m=1:nv= \
      --slot n=2:m=2:nv= \
      --slot n=3:m=3:nv=  -- ${cfg} | tee ${logdir}/output.log
...
run_ofr3() {
  for config_name in "${ofr3_config_names[@]}"; do
    jobs=4
    threads=64
    streams=64
    events=1000
    logdir="logs.Bergamo.OFR3.$config_name.${jobs}j.${threads}t.${streams}s"
