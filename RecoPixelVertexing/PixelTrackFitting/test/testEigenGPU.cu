...
__global__
void kernelFullFit(Rfit::Matrix3xNd<4> * hits,
    Rfit::Matrix3Nd<4> * hits_cov,
    double B,
    bool errors,
  ...
  Rfit::Matrix2xNd<N> hits2D_local = (hits->block(0,0,2,n)).eval();
  Rfit::Matrix2Nd<N> hits_cov2D_local = (hits_cov->block(0, 0, 2 * n, 2 * n)).eval();
...
__global__
void kernelFastFit(Rfit::Matrix3xNd<4> * hits, Vector4d * results) {
  Rfit::Fast_fit(*hits, *results);
}

__global__
void kernelCircleFit(Rfit::Matrix3xNd<4> * hits,
    Rfit::Matrix3Nd<4> * hits_cov, Vector4d * fast_fit_input, double B,
    Rfit::circle_fit * circle_fit_resultsGPU) {
  u_int n = hits->cols();
  constexpr uint32_t N = 4;
  Rfit::VectorNd<N> rad = (hits->block(0, 0, 2, n).colwise().norm());
  ...
  __global__
void kernelLineFit(Rfit::Matrix3xNd<4> * hits,
    Rfit::Matrix3Nd<4> * hits_cov,
    ...
    double B)
{
  (*line_fit) = Rfit::Line_fit(*hits, *hits_cov, *circle_fit, *fast_fit, B, true);
}

void fillHitsAndHitsCov(Rfit::Matrix3xNd<4> & hits, Rfit::Matrix3Nd<4> & hits_cov) {
...
void testFit() {
  ...
  Rfit::Matrix3Nd<N> hits_cov = MatrixXd::Zero(12,12);
  Rfit::Matrix3xNd<N> * hitsGPU = new Rfit::Matrix3xNd<N>;
  Rfit::Matrix3Nd<N> * hits_covGPU = nullptr;
  ...
  // FAST_FIT GPU
  cudaMalloc((void**)&hitsGPU, sizeof(Rfit::Matrix3xNd<N>));
  cudaMalloc((void**)&fast_fit_resultsGPU, sizeof(Vector4d));
  cudaMemcpy(hitsGPU, &hits, sizeof(Rfit::Matrix3xNd<N>), cudaMemcpyHostToDevice);
  ...
  Rfit::Matrix2xNd<N> hits2D_local = (hits.block(0,0,2,n)).eval();
  Rfit::Matrix2Nd<N> hits_cov2D_local = (hits_cov.block(0, 0, 2 * n, 2 * n)).eval();
  Rfit::circle_fit circle_fit_results = Rfit::Circle_fit(hits2D_local,
      hits_cov2D_local,
      fast_fit_results, rad, B, false);
  std::cout << "Fitted values (CircleFit):\n" << circle_fit_results.par << std::endl;

  // CIRCLE_FIT GPU
  cudaMalloc((void **)&hits_covGPU, sizeof(Rfit::Matrix3Nd<N>));
  cudaMalloc((void **)&circle_fit_resultsGPU, sizeof(Rfit::circle_fit));
  cudaMemcpy(hits_covGPU, &hits_cov, sizeof(Rfit::Matrix3Nd<N>), cudaMemcpyHostToDevice);
  ...
  // LINE_FIT CPU
  Rfit::line_fit line_fit_results = Rfit::Line_fit(hits, hits_cov, circle_fit_results, fast_fit_results, B, true);
  ...
  kernelLineFit<<<1,1>>>(hitsGPU, hits_covGPU, circle_fit_resultsGPU, fast_fit_resultsGPU, line_fit_resultsGPU, B);
  cudaDeviceSynchronize();
...
void testFitOneGo(bool errors, double epsilon=1e-6) {
  ...
  Rfit::Matrix3xNd<N> * hitsGPU = nullptr; // new Rfit::Matrix3xNd(3,4);
  Rfit::Matrix3Nd<N> * hits_covGPU = nullptr;
  Rfit::line_fit * line_fit_resultsGPU = nullptr;
  Rfit::line_fit * line_fit_resultsGPUret = new Rfit::line_fit();
  ...
  cudaCheck(cudaMalloc((void **)&hitsGPU, sizeof(Rfit::Matrix3xNd<N>)));
  cudaCheck(cudaMalloc((void **)&hits_covGPU, sizeof(Rfit::Matrix3Nd<N>)));
  ...
  cudaCheck(cudaMemcpy(hitsGPU, &hits, sizeof(Rfit::Matrix3xNd<N>), cudaMemcpyHostToDevice));
  cudaCheck(cudaMemcpy(hits_covGPU, &hits_cov, sizeof(Rfit::Matrix3Nd<N>), cudaMemcpyHostToDevice));
