                   ...
                   Rfit::line_fit * line_fit_resultsGPU)
{
    ...
    Rfit::printIt(hits, "KernelFullFit - input hits: ");
    ...
    (*circle_fit_resultsGPU) =
        Rfit::Circle_fit(hits->block(0,0,2,n), hits_cov->block(0, 0, 2 * n, 2 * n), fast_fit, rad, B, errors);
    /*
       (*circle_fit_resultsGPU) =
...
#endif
    (*circle_fit_resultsGPU) =
        Rfit::Circle_fit(hits->block(0,0,2,n), hits_cov->block(0, 0, 2 * n, 2 * n), *fast_fit_input, rad, B, false);
}
...
void testFit() {
    ...
    kernelCircleFit<<<1,1>>>(hitsGPU, hits_covGPU, fast_fit_resultsGPU, B, circle_fit_resultsGPU);
    cudaDeviceSynchronize();
...
void testFitOneGo(bool errors, double epsilon = 1e-6) {
    ...
    kernelFullFit<<<1, 1>>>(hitsGPU, hits_covGPU, B, errors, circle_fit_resultsGPU, line_fit_resultsGPU);
...
int main(int argc, char * argv[]) {
    ...
    std::cout << "TEST BROKEN LINE FIT" << std::endl;
