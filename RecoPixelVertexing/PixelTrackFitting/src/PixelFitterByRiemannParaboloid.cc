...
float chi2_circle(Matrix<float, 2, Dynamic, 0, 2, max_nop> points2D,
                  Matrix<float, Dynamic, Dynamic, 0, 2 * max_nop, 2 * max_nop> V,
                  Vector3f par_uvr) {
  float chi2 = 0;
  ...
  for (int i = 0; i < nop; i++) {
    ...
    float y_2 = sqr(y_);
    chi2 +=
        sqr(sqrt(x_2 + y_2) - par_uvr(2)) /
        ((V(i, i) * x_2 + V(i + nop, i + nop) * y_2 + 2 * V(i, i + nop) * x_ * y_) / (x_2 + y_2));
...
float chi2_line(Matrix<float, 2, Dynamic, 0, 2, max_nop> points2D,
  ...
  int nop = points2D.cols();
  for (int i = 0; i < nop; i++) {
    chi2 += sqr(points2D(1, i) - points2D(0, i) * par_line(0) - par_line(1)) /
            (1 + sqr(par_line(0))) /
...
inline int Charge(Matrix<float, 2, Dynamic, 0, 2, max_nop> points2D,
                  Vector3f par_uvr) {  // error to be computed TO FIX
...
Vector3f par_transformation(Vector3f par_uvr, int charge, float B_field) {
  ...
  par_pak << phi, charge * (sqrt(sqr(par_uvr(0)) + sqr(par_uvr(1))) - par_uvr(2)),
      par_uvr(2) * B_field;
  return par_pak;
}
...
inline float X(Matrix<float, 5, 1> par) {
  return 2.f *
         asin(sqrt(sqr(par(3) - cos(par(0) - halfpi) * par(1)) +
                   sqr(par(4) - sin(par(0) - halfpi) * par(1))) *
              par(2) / 2.f) /
         par(2);
}
...
circle_fit Circle_fit(Matrix<float, 2, Dynamic, 0, 2, max_nop> points2D,
  ...
  Matrix<float, Dynamic, 1, 0, max_nop, 1> weight =
      1.f / (V.block(0, 0, nop, nop).diagonal() + V.block(nop, nop, nop, nop).diagonal()).array();
  weight /= weight.sum();
  ...
  Vector3f par_uvr_;
  par_uvr_ << -n(0) / (2.f * n(2)), -n(1) / (2.f * n(2)),
  ...
  if (return_err) {
    ...
    Matrix<float, Dynamic, Dynamic, 0, max_nop, max_nop> H =
        MatrixXf::Identity(nop, nop) - VectorXf::Constant(nop, 1.f) * weight.transpose();
    ...
    // Matrix<float, Dynamic, Dynamic, 0, 2*max_nop, 2*max_nop> J1 = s *
    // (MatrixXf::Identity(2*nop,2*nop) - mc*mc.transpose()/q);  MatrixXf Vcs_ = J1 * V *
    // J1.transpose();

    Matrix<float, Dynamic, Dynamic, 0, 2 * max_nop, 2 * max_nop> Vcs =
        sqr(s) * V + pow(s, 4) / sqr(b) *
                         (sqr(1 / (2.f * sqrt(q * nop))) *
                          (2.f * V.array().square().sum() + 4.f * mc.transpose() * V * mc)) *
                         mc * mc.transpose();

    Matrix<float, Dynamic, Dynamic, 0, 3 * max_nop, 3 * max_nop> C =
        MatrixXf::Zero(3 * nop, 3 * nop);
    ...
    for (int i = 0; i < 2; i++)
      for (int j = 0; j < 2; j++)
        C.block(2 * nop, 2 * nop, nop, nop) +=
            (2.f * Vcs.block(i * nop, i * nop, nop, nop).array() *
                 Vcs.block(i * nop, j * nop, nop, nop).array() +
             4.f * Vcs.block(i * nop, j * nop, nop, nop).array() *
                 (points3D.row(i).transpose() * points3D.row(j)).array())
                .matrix();

    Matrix<float, Dynamic, Dynamic, 0, 3 * max_nop, 3 * max_nop> D =
        MatrixXf::Zero(3 * nop, 3 * nop);
    for (int i = 0; i < 3; i++) {
      for (int j = i; j < 3; j++) {
        D.block(i * nop, j * nop, nop, nop) =
            H * C.block(i * nop, j * nop, nop, nop) * H;  // H.transpose();
        if (i != j)
          D.block(j * nop, i * nop, nop, nop) = D.block(i * nop, j * nop, nop, nop).transpose();
      }
    }
    ...
    for (int a = 0; a < 6; a++) {
      for (int z = a; z < 6; z++) {
        ...
        nu(z, &k, &l);
        E(a, z) =
            (D.block(i * nop, k * nop, nop, nop).array() * W.array() *
                 D.block(j * nop, l * nop, nop, nop).array() +
             D.block(i * nop, l * nop, nop, nop).array() * W.array() *
                 D.block(j * nop, k * nop, nop, nop).array())
                .sum() +
            (s_v.col(i).transpose() *
             (D.block(j * nop, l * nop, nop, nop).array() * W.array()).matrix() * s_v.col(k)) +
            (s_v.col(i).transpose() *
             (D.block(j * nop, k * nop, nop, nop).array() * W.array()).matrix() * s_v.col(l)) +
            (s_v.col(j).transpose() *
             (D.block(i * nop, l * nop, nop, nop).array() * W.array()).matrix() * s_v.col(k)) +
            (s_v.col(j).transpose() *
             (D.block(i * nop, k * nop, nop, nop).array() * W.array()).matrix() * s_v.col(l));
        E(z, a) = E(a, z);
      }
    }
    ...
    Matrix<float, 3, 4> J3;
    J3 << -1.f / (2.f * n(2)), 0, n(0) / (2.f * sqr(n(2))), 0, 0, -1.f / (2 * n(2)),
        n(1) / (2.f * sqr(n(2))), 0, 0, 0,
        -h / (2.f * sqr(n(2))) - (4.f * c + 2.f * n(2)) / (4.f * h * n(2)), -1.f / h;

    Matrix3f cov_uvr = (J3 * Cnc * J3.transpose()) / sqr(s) +
                       (par_uvr_ * par_uvr_.transpose()) * (Jq * V * Jq.transpose()) / sqr(b);

    Matrix3f J4 = Matrix3f::Zero();
    J4 << -par_uvr(1) / sqr(par_uvr(0)) * 1.f / (1 + sqr(par_uvr(1) / par_uvr(0))),
        1.f / par_uvr(0) * 1.f / (1 + sqr(par_uvr(1) / par_uvr(0))), 0,
        circle.charge * par_uvr(0) / sqrt(sqr(par_uvr(0)) + sqr(par_uvr(1))),
        circle.charge * par_uvr(1) / sqrt(sqr(par_uvr(0)) + sqr(par_uvr(1))), -circle.charge, 0, 0,
        B_field;
    ...
    for (int a = 0; a < 3; a++) {
      ...
      J4_.col(a) =
          (par_transformation(par_uvr + Delta, circle.charge, B_field) - circle.par) / Delta(a);
    }
...
line_fit Line_fit(Matrix<float, 3, Dynamic, 0, 3, max_nop> points3D,
  ...
  Matrix<float, Dynamic, 1, 0, max_nop, 1> y_err2 = V.block(2 * nop, 2 * nop, nop, nop).diagonal();
  ...
  points2D.row(0) =
      2.f *
      asin(((points3D.row(0).array() - cos(circle.par(0) - halfpi) * circle.par(1)).square() +
            (points3D.row(1).array() - sin(circle.par(0) - halfpi) * circle.par(1)).square())
               .sqrt() /
           (circle.par(2) / B_field * 2.f)) *
      (circle.par(2) / B_field);
  ...
  if (return_err) {
    ...
    Matrix3f C;
    C << sig2 * n1_2 / S, -sig2 * n(0) * n(1) / S, C13, -sig2 * n(0) * n(1) / S, sig2 * n0_2 / S,
        C23, C13, C23, C33;

    Matrix<float, 2, 3> J;
    J << -1.f / n(1), n(0) / n1_2, 0, -c * n(0) / (n(1) * sqrt_), n0_2 * c / (n1_2 * sqrt_),
        -sqrt_ / n(1);
...
helix_fit Helix_fit(Matrix<float, 3, Dynamic, 0, 3, max_nop> hits,
                    Matrix<float, Dynamic, Dynamic, 0, 3 * max_nop, 3 * max_nop> hits_cov,
                    float B_field, bool return_err = true) {
  int nop = hits.cols();
...
std::unique_ptr<reco::Track> PixelFitterByRiemannParaboloid::run(
  ...
  Matrix<float, Dynamic, Dynamic, 0, 3 * max_nop, 3 * max_nop> riemannHits_cov =
      MatrixXf::Zero(3 * nhits, 3 * nhits);
