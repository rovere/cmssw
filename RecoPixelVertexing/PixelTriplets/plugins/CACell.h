...
CACell(const HitDoublets* doublets, int doubletId, /*const unsigned int cellId,*/
         const int innerHitId, const int outerHitId)
      : theCAState(0),
        theInnerHitId(innerHitId),
        theOuterHitId(outerHitId),
        /*theCellId(cellId),*/ hasSameStateNeighbors(0),
        theDoublets(doublets),
        theDoubletId(doubletId),
        theInnerR(doublets->r(doubletId, HitDoublets::inner)),
        theOuterR(doublets->r(doubletId, HitDoublets::outer)),
        theInnerZ(doublets->z(doubletId, HitDoublets::inner)),
        theOuterZ(doublets->z(doubletId, HitDoublets::outer)) {}
  /*
  ...
  */
  ...
  void check_alignment_and_tag(CACell* innerCell, const float ptmin,
    ...
    if (are_aligned_RZ(innerCell, ptmin, thetaCut) &&
        have_similar_curvature(innerCell, region_origin_x, region_origin_y,
  ...
  bool are_aligned_RZ(const CACell* otherCell, const float ptmin,
    ...
    float tan_12_13_half =
        fabs(z1 * (theInnerR - theOuterR) + theInnerZ * (theOuterR - r1) +
             theOuterZ * (r1 - theInnerR)) /
        distance_13_squared;
  ...
  bool have_similar_curvature(const CACell* otherCell,
    ...
    // points are aligned
    if (fabs(det) < precision) return true;
  ...
  unsigned int get_CA_state() const { return theCAState; }
  // if there is at least one left neighbor with the same state (friend), the
  // state has to be increased by 1.
  ...
  // the visit of the graph
  // based on the neighborhood connections between cells.

  void find_ntuplets(std::vector<CAntuplet>& foundNtuplets,
    ...
    if (theOuterNeighbors.size() == 0) {
      ...
      for (unsigned int i = 0; i < numberOfOuterNeighbors; ++i) {
        ...
        theOuterNeighbors[i]->find_ntuplets(foundNtuplets, tmpNtuplet,
                                            minHitsPerNtuplet);
        tmpNtuplet.pop_back();
      }
    }
