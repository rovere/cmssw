...
class CAHitQuadrupletGenerator : public HitQuadrupletGenerator {
 ...
 public:
  ...
  /// from base class
  virtual void hitQuadruplets(const TrackingRegion& reg,
                              OrderedHitSeeds& triplets, const edm::Event& ev,
                              const edm::EventSetup& es);

  void findQuadruplets(const TrackingRegion& region, OrderedHitSeeds& result,
                       const edm::Event& ev, const edm::EventSetup& es,
                       const std::unordered_map<int, int>&,
                       const std::map<std::pair<int, int>, HitDoublets>&);
  ...
  class QuantityDependsPtEval {
   public:
    ...
    float value(float curvature) const {
      ...
      if (curvature1_ < curvature) return value1_;
      if (curvature2_ < curvature && curvature <= curvature1_)
        return value2_ +
               (curvature - curvature2_) / (curvature1_ - curvature2_) *
                   (value1_ - value2_);
      return value2_;
    }
  ...
  class QuantityDependsPt {
   public:
    explicit QuantityDependsPt(const edm::ParameterSet& pset)
      ...
      if (pt1_ <= 0)
        throw cms::Exception("Configuration")
            << "PixelQuadrupletGenerator::QuantityDependsPt: pt1 needs to be > "
               "0; is "
            << pt1_;
      if (pt2_ <= 0)
        throw cms::Exception("Configuration")
            << "PixelQuadrupletGenerator::QuantityDependsPt: pt2 needs to be > "
               "0; is "
            << pt2_;
    }
    ...
    QuantityDependsPtEval evaluator(const edm::EventSetup& es) const {
      if (enabled_) {
        return QuantityDependsPtEval(
            value1_, value2_, PixelRecoUtilities::curvature(1.f / pt1_, es),
