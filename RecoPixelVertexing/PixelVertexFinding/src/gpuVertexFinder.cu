...
namespace gpuVertexFinder {
  ...
  void Producer::produce(cudaStream_t stream, TuplesOnCPU const & tracks, float ptMin) {
    ...
    assert(gpu_d);assert(ws_d);assert(tracks.gpu_d);
  ...
  Producer::OnCPU const & Producer::fillResults(cudaStream_t stream) {
    ...
    cudaCheck(cudaMemcpyAsync(gpuProduct.ivtx.data(),LOC_WS(iv),sizeof(int32_t)*gpuProduct.nTracks,
                              cudaMemcpyDeviceToHost, stream));
    gpuProduct.itrk.resize(gpuProduct.nTracks);
    cudaCheck(cudaMemcpyAsync(gpuProduct.itrk.data(),LOC_WS(itrk),sizeof(int16_t)*gpuProduct.nTracks,
                              cudaMemcpyDeviceToHost, stream));

    gpuProduct.z.resize(gpuProduct.nVertices);
    cudaCheck(cudaMemcpyAsync(gpuProduct.z.data(),LOC_ZV(zv),sizeof(float)*gpuProduct.nVertices,
			      cudaMemcpyDeviceToHost, stream));
    gpuProduct.zerr.resize(gpuProduct.nVertices);
    cudaCheck(cudaMemcpyAsync(gpuProduct.zerr.data(),LOC_ZV(wv),sizeof(float)*gpuProduct.nVertices,
			      cudaMemcpyDeviceToHost, stream));
    gpuProduct.chi2.resize(gpuProduct.nVertices);
    cudaCheck(cudaMemcpyAsync(gpuProduct.chi2.data(),LOC_ZV(chi2),sizeof(float)*gpuProduct.nVertices,
			      cudaMemcpyDeviceToHost, stream));
        
    gpuProduct.sortInd.resize(gpuProduct.nVertices);
    cudaCheck(cudaMemcpyAsync(gpuProduct.sortInd.data(),LOC_ZV(sortInd),sizeof(uint16_t)*gpuProduct.nVertices,
                              cudaMemcpyDeviceToHost, stream));
