#ifndef RecoPixelVertexing_PixelVertexFinding_clusterTracksByDensity_H
#define RecoPixelVertexing_PixelVertexFinding_clusterTracksByDensity_H
...
namespace gpuVertexFinder {
  ...
  void clusterTracksByDensity(
   ...
   //  mini verification
   for (auto i = threadIdx.x; i < nt; i += blockDim.x) {
    if (iv[i]!=int(i)) assert(iv[iv[i]]!=int(i));  
   }
   __syncthreads();
#endif
