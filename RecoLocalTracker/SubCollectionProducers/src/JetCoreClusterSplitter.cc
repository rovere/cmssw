...
// bool pixelPosSort (const SiPixelCluster &  i, const SiPixelCluster & j) { if
// (i.x<j.x) return true; return i.y<j.y; }
...
std::vector<SiPixelCluster> JetCoreClusterSplitter::fittingSplit(
  ...
  while (!stop && remainingSteps > 0) {
    ...
    for (std::multimap<float, int>::iterator it = scores.begin();
      ...
      int j = it->second;
      if (verbose)
        std::cout << "Original Pixel " << j << " with score " << it->first << std::endl;
      ...
      for (auto subpixel = pixels.begin(); subpixel != pixels.end();
        ...
        if (subpixel->first > j) {
          ...
          if (verbose)
            std::cout << "Pixel weight j cl " << weightOfPixel[subpixel_counter] << " " << subpixel_counter
                      << " " << cl << std::endl;
        }
      }
    }
    ...
    for (unsigned int j = 0; j < pixels.size(); j++) {
      ...
      if (verbose)
        std::cout << "j " << j << " x " << pixels[j].second.x << " * y "
                  << pixels[j].second.y << " * ADC " << pixels[j].second.adc << " * W "
                  << weightOfPixel[j] << std::endl;
      clx[clusterForPixel[j]] += pixels[j].second.x * pixels[j].second.adc;
      cly[clusterForPixel[j]] += pixels[j].second.y * pixels[j].second.adc;
      cls[clusterForPixel[j]] += pixels[j].second.adc;
      //			std::cout << "update cluster " <<
      //clusterForPixel[j] << " x,y " << clx[clusterForPixel[j]] << " " <<
      //cly[clusterForPixel[j]] <<  "weight x,y " << cls[clusterForPixel[j]] <<
      //" " << clx[clusterForPixel[j]]/cls[clusterForPixel[j]] << " " <<
      //cly[clusterForPixel[j]]/cls[clusterForPixel[j]] <<std::endl;
    }
    for (unsigned int i = 0; i < meanExp; i++) {
      if (cls[i] != 0) {
        clx[i] /= cls[i];
      ...
      if (verbose)
        std::cout << "Center for cluster " << i << " x,y " << clx[i] << " "
                  << cly[i] << std::endl;
      cls[i] = 0;
    }
  }
