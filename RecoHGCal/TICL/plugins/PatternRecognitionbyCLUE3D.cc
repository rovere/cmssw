  ...
  for (unsigned int i = 0; i < numberOfClusters; i++) {
    ...
    for (int currentLayer = minLayer; currentLayer <= maxLayer; currentLayer++) {
      std::cout << "RefLayer: " << layerId << " SoaIDX: " << i
         << "NextLayer: " << currentLayer << std::endl;
      if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > PatternRecognitionAlgoBaseT<TILES>::Advanced) {
      ...
      int etaWindow = onSameLayer ? 2 : 2;
      int phiWindow = onSameLayer ? 2 : 2;
  ...
  for (unsigned int layer = 0; layer < 2 * rhtools_.lastLayer(); layer++) {
    ...
    for (unsigned int i = 0; i < numberOfClusters; i++) {
      ...
      bool isSeed = (
          ...
          ) || (clustersOnLayer.rho[i] > 0.8 && clustersOnLayer.energy[i] < clustersOnLayer.rho[i] && clustersOnLayer.delta[i] > criticalEtaPhiDistance_);
      ...
      if (isSeed) {
        ...
        if (lyrIdx >=0)
          ...
          std::cout
              << "Found fake follower on Layer " << layer << " SOAidx: " << i << " attached to cluster on layer: " << lyrIdx
              << " SOAidx: " << soaIdx
              << "ClusterInfo: "  << " x " << clustersOnLayer.x[i] << " y " << clustersOnLayer.y[i] << ", "
              << " eta " << clustersOnLayer.eta[i] << " phi " << clustersOnLayer.phi[i] << " cells " << clustersOnLayer.cells[i] << " energy "
              << clustersOnLayer.energy[i] << " radius " << clustersOnLayer.radius[i] << " rho " << clustersOnLayer.rho[i] << " delta "
              << clustersOnLayer.delta[i] << " isSeed " << clustersOnLayer.isSeed[i] << " clusterIdx " << clustersOnLayer.clusterIndex[i] << " ClusterOrigIdx "
              << clustersOnLayer.layerClusterOriginalIdx[i] << std::endl;
      } else {
        if (PatternRecognitionAlgoBaseT<TILES>::algo_verbosity_ > PatternRecognitionAlgoBaseT<TILES>::Advanced) {
          edm::LogVerbatim("PatternRecogntionbyCLUE3D")
