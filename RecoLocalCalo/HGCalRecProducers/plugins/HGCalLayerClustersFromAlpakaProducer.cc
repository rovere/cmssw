...
class HGCalLayerClustersFromAlpakaProducer : public edm::stream::EDProducer<> {
  ...
  void produce(edm::Event& iEvent, edm::EventSetup const& iSetup) override {
    ...
    unsigned int numberOfClusters = clustersSoA.view().numberOfClusters()[0];
    std::cout << "Before create clusters" << std::endl;
    *clusters = createClusters(numberOfClusters, cells.view().cellsCout(), clustersSoA, cells, times);
  ...
  private:
    ...
    std::vector<reco::BasicCluster> createClusters(int numberOfClusters, int numberOfCells, const HGCalSoAOutHostCollection &clustersSoA, const HGCalSoACellsHostCollection &cells, std::vector<std::pair<float, float>> &times){
      ...
      for (int i = 0; i < numberOfCells; i++){
          auto clusterSoA = clustersSoAView[i];
          auto cell = cellsView[i];
          if (clusterSoA.clusterIndex() == -1)
              continue;
          int globalClusterIdx = clusterSoA.clusterIndex();
          if (usedIdx.find(globalClusterIdx) != usedIdx.end()){
              clusters[globalClusterIdx].setEnergy(clusters[globalClusterIdx].energy() + cell.weight());
              clusters[globalClusterIdx].addHitAndFraction(cell.detid(), 1.f);
          }
          else{
              std::vector<std::pair<DetId, float>> thisCluster;
              thisCluster.emplace_back(cell.detid(), 1.f);
              math::XYZPoint position = math::XYZPoint(0.f, 0.f, 0.f);
              clusters[globalClusterIdx] = reco::BasicCluster(cell.weight(), position, reco::CaloID::DET_HGCAL_ENDCAP, std::move(thisCluster), algoId_ );
              usedIdx.emplace(globalClusterIdx);
          }
          if (clusterSoA.isSeed())
              clusters[globalClusterIdx].setSeed(cell.detid());
      }
